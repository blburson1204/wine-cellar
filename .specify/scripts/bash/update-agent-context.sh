...
validate_environment() {
    ...
    if [[ ! -f "$NEW_PLAN" ]]; then
        ...
        log_info "Auto-detected spec: $CURRENT_BRANCH"
        log_info ""
        log_info "To use a different spec, set SPECIFY_FEATURE:"
        log_info "  SPECIFY_FEATURE=NNN-feature-name $0 $AGENT_TYPE"
        log_info ""
        log_info "Available specs with plan.md:"
        for spec_dir in "$REPO_ROOT/specs"/*; do
            if [[ -f "$spec_dir/plan.md" ]]; then
                log_info "  - $(basename "$spec_dir")"
            fi
        done
        exit 1
    fi
